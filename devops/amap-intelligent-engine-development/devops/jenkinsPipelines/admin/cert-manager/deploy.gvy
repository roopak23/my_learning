            ...
            steps {
                script {                       
                    ...
                    dir("${workspace}/appconfig/${env.ENVIRONMENT}") {
                        env.AWS_REGION=sh(script: ''' yq  e  '.params[] | select(.name=="PARAMS.AWS_REGION").value' cert-manager.yaml ''' , returnStdout: true).trim()
                        env.EKS_CLUSTER_NAME=sh(script: ''' yq  e  '.params[] | select(.name=="PARAMS.EKS_CLUSTER_NAME").value' cert-manager.yaml ''' , returnStdout: true).trim()
                        
                    }
                }
            }
        }
        stage("Configure cert-manager") {
            steps {
                dir("${workspace}/charts/cert-manager") {
                    script {
                        sh """
                ...
                dir("${workspace}/charts/cert-manager") {
                    script {
                        sh """
                            ...
                            helm upgrade -n cert-manager --install cert-manager . --values=values-${env.ENVIRONMENT}.yaml --create-namespace \${command_param}

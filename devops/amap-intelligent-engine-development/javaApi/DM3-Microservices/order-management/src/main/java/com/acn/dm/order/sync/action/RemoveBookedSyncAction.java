...
@Slf4j
@Component
public class RemoveBookedSyncAction extends AbstractOrderSyncAction {

	private final QueueService queueService;


	public RemoveBookedSyncAction(APIReservedHistoryRepository histRepo, ApiInventoryCheckRepository checkRepo,
	...
	protected OrderSyncActions canHandleAction() {
		return OrderSyncActions.REMOVE_FROM_BOOKED;
	}
	...
	public void handle(MarketOrderLineDetailsRequest request, Map<LocalDateTime, Collection<QuantityAndPeriod>> splittedList) throws Exception {
		...
		List<ApiInventoryCheck> invDate = customQueryService.findEntitiesByKey(apiInventoryCheckPK)
				...
				.toList();
		decreaseInvBooked(reserveHistData, invDate);
		...
		if (!reserveHistData.isEmpty() && !invDate.isEmpty()) {
			...
			moldHistoryRepository.findById(request.getMoldId())
					.ifPresent(value -> moldHistoryRepository.save(removeBookedMoldHistory(value, request)));

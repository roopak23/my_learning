...
import com.acn.dm.order.mappers.OrderSyncToInventoryCheckMapper;
...
@Slf4j
@Component
public class AddBookedSyncAction extends AbstractOrderSyncAction {

  @Autowired
  AbstractInventoryCheckRequest checkRequest;

  private final InventoryManagementFeignClient inventory;
  private final OrderSyncToInventoryCheckMapper inventoryRequestMapper;
   ...
   * @param queueService
   */
  public AddBookedSyncAction(APIReservedHistoryRepository histRepo, ApiInventoryCheckRepository checkRepo,
                             SplitConfigs dateSplitter, ApiMoldDataRepository moldDataRepository, ApiMoldHistoryRepository moldHistoryRepository,
                             InventoryManagementFeignClient inventory, OrderSyncToInventoryCheckMapper inventoryRequestMapper, CustomQueryService customQueryService, QueueService queueService) {
  ...
  protected OrderSyncActions canHandleAction() {
    return OrderSyncActions.ADD_BOOKED;
  }
  ...
  public void handle(MarketOrderLineDetailsRequest request, Map<LocalDateTime, Collection<QuantityAndPeriod>> splittedList) throws Exception {
    ...
    List<ApiReservedHistory> currentlyReservedValues = splittedList.entrySet()
            ...
            .map(qAp -> apiHistFromQuantityAndPeriod(qAp, request, OrderSyncActions.ADD_BOOKED))
            .toList();
    ...
    if (!currentlyReservedValues.isEmpty() && !currentlyInventoryCheckValues.isEmpty()) {
      ...
      moldHistoryRepository.save(createMoldHistory(request, OrderSyncActions.ADD_BOOKED));
  ...
  public void saveMoldHistoryFullyClash(MarketOrderLineDetailsRequest request) {
    moldHistoryRepository.save(decorateClashMoldHistory(createMoldHistory(request, OrderSyncActions.ADD_BOOKED)));
  }

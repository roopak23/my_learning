...
@Component
public class UpdateReservedSyncAction extends AbstractOrderSyncAction {

    private final QueueService queueService;


    public UpdateReservedSyncAction(APIReservedHistoryRepository histRepo, ApiInventoryCheckRepository checkRepo,
    ...
    protected OrderSyncActions canHandleAction() {
        return OrderSyncActions.UPDATE_RESERVED;
    }
    ...
    public void handle(MarketOrderLineDetailsRequest request, Map<LocalDateTime, Collection<QuantityAndPeriod>> splittedList) throws Exception {
        ...
        if (!currentlyReservedValues.isEmpty() && !currentlyInventoryCheckValues.isEmpty()) {
            ...
            moldHistoryRepository.findById(request.getMoldId())
                    .ifPresent(value -> moldHistoryRepository.save(updateMoldHistory(value, request, OrderSyncActions.UPDATE_RESERVED)));
            if (!moldData.isEmpty()) {

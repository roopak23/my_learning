...
@Slf4j
@Component
public class MoveFromReservedToBookedAndSumBookedSyncAction extends AbstractOrderSyncAction {

    private final QueueService queueService;


    public MoveFromReservedToBookedAndSumBookedSyncAction(APIReservedHistoryRepository histRepo, ApiInventoryCheckRepository checkRepo,
    ...
    protected OrderSyncActions canHandleAction() {
        return OrderSyncActions.MOVE_RESERVED_BOOKED_TO_BOOKED;
    }
...
//      Find and remove old reservation
        ...
        List<ApiReservedHistory> reservedToUpdate = reserveHistDataRemove.stream()
                ...
                .peek(res -> res.setQuantity(res.getQuantityBooked()))
...
//      Move to booked new reservation
        ...
        List<ApiReservedHistory> reserveCombineValues  = histRepo.findByMoldId(request.getMoldId());
        reserveCombineValues.addAll(currentlyReservedValues);

        Map<APIReservedHistoryPK, ApiReservedHistory> reserveCombineResults = reserveCombineValues.stream()
                .peek(res -> res.setStatus(OrderStatus.READY.getValue()))
                .collect(Collectors.toMap(ApiReservedHistory::getId,
                        value -> value, (existing, replacement) -> {
                            replacement.setQuantityReserved(0);
                            replacement.setQuantity(replacement.getQuantityBooked() + existing.getQuantityBooked());
        ...
        if (!reserveCombineResults.isEmpty()) {
            ...
            moldHistoryRepository.findById(request.getMoldId())
                    .ifPresent(value -> moldHistoryRepository.save(moveMoldHistoryReady(value, request)));
    ...
    public void saveMoldHistoryFullyClash(MarketOrderLineDetailsRequest request) {
        moldHistoryRepository.findById(request.getMoldId())
                .ifPresent(value -> moldHistoryRepository.save(moveMoldHistoryReadyClash(value, request)));
    }
}

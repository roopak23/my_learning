...
@Slf4j
@Component
public class RemoveReservedAndBookedSyncAction extends AbstractOrderSyncAction {

    private final QueueService queueService;


    public RemoveReservedAndBookedSyncAction(APIReservedHistoryRepository histRepo, ApiInventoryCheckRepository checkRepo,
    ...
    protected OrderSyncActions canHandleAction() {
        return OrderSyncActions.REMOVE_BOOKED_RESERVED;
    }
    ...
    public void handle(MarketOrderLineDetailsRequest request, Map<LocalDateTime, Collection<QuantityAndPeriod>> splittedList) throws Exception {
        ...
        decreaseInvReservedAndBooked(reserveHistData, invDate);
        ...
        if (!reserveHistData.isEmpty() && !invDate.isEmpty()) {
            ...
            moldHistoryRepository.findById(request.getMoldId())
                    .ifPresent(value -> moldHistoryRepository.save(removeReservedBookedMoldHistory(value, request)));
    ...
    private void mapStatus(ApiReservedHistory reservedHistory, String status) {
        ...
        reservedHistory.setQuantityBooked(0);
    }
    ...
    public void saveMoldHistoryFullyClash(MarketOrderLineDetailsRequest request) {
        moldHistoryRepository.findById(request.getMoldId())
                .ifPresent(value -> moldHistoryRepository.save(decorateClashMoldHistoryY(removeMoldHistory(value, request))));
    }
}

SELECT
	...
	`adserver_name`,
	`bid_strategy`,
	round(round(t2.`actual_pacing`,4),3) as `actual_pacing`,
	`budget`,
	`target_kpi`,
	`target_kpi_value`,
	`impressions_act`,
	...
	`order_adslot_level`
FROM TRF_Monitoring_Insights t1 
left join (select actual_pacing,tech_line_remote_id  from TRF_Monitoring_Insights where partition_date = {{ params.ENDDATE }} and `rnk` = 1 and upper(trim(report_id)) = 'REPORT0') t2 
on t1.tech_line_remote_id= t2.tech_line_remote_id
where t1.partition_date = {{ params.ENDDATE }} and t1.`rnk` = 1
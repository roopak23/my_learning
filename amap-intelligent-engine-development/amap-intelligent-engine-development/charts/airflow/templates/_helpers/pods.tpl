...
{{- define "airflow.container.custom_git_sync" }}
- name: git-sync
  image: {{ .Values.airflow.image.repository }}:git-latest
  securityContext:
    runAsUser: 0
  command:
  - "/bin/sh"
  ...
  - |
    ...
    apk add rsync
    GIT_URL={{ .Values.dags.customGitSync.url }}
    BRANCH_NAME=development
    SPARK_DIR=Run/Spark
    PIPELINES_DIR=Run/Airflow
    ...
    echo $SPARK_DIR/ >> .git/info/sparse-checkout;
    echo $PIPELINES_DIR/ >> .git/info/sparse-checkout;
    ...
    echo "Making destination folders";
    mkdir -p /opt/airflow/dags/Spark/
    mkdir -p /opt/airflow/dags/scripts/batch/pipelines/

    while true; do
      git fetch origin $BRANCH_NAME;
      git reset --hard origin/$BRANCH_NAME;
      git clean -fd;
      rsync -ru --progress /tmp/repo/$SPARK_DIR/* /opt/airflow/dags/Spark/ 
      rsync -ru --progress /tmp/repo/$PIPELINES_DIR/dags/scripts/batch/pipelines/* /opt/airflow/dags/scripts/batch/pipelines/
      date;
      sleep 60;
    done
  ...
  volumeMounts:
    - name: dags-spark
      mountPath: opt/airflow/dags/Spark
    - name: dags-pipelines
      mountPath: opt/airflow/dags/scripts/batch/pipelines
{{- end }}
...
- name: dags-data
  mountPath: {{ .Values.dags.path }}
{{- else if .Values.dags.customGitSync.enabled }}
- name: dags-spark
  mountPath: {{ .Values.dags.path }}/Spark
- name: dags-pipelines
  mountPath: {{ .Values.dags.path }}/scripts/batch/pipelines
{{- end }}

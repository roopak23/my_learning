...
# ======== DAG ========
with DAG('CRM_Inventory_management', default_args=default_args, schedule_interval='15 20 * * *', tags=['CRM_Inventory_management'], template_searchpath=[], max_active_runs=1,catchup =  False) as dag:
    ...
    with TaskGroup("Preparation") as preparation_steps:
        ...
        copy_to_machine = SFTPBatchOperator(
            ...
            local_folder=dag_conf['DAG']['FILE_PATH'] + '/CRM_Inventory_management/',
            remote_folder=f"/tmp/CRM_Inventory_management/",
    ...
    with TaskGroup("Execution", tooltip="Run Data Extraction Script") as execution_steps:
        ...
        run_script = SSHOperator(
            ...
            ssh_conn_id='my_emr_ssh',
            command='spark-submit /tmp/CRM_Inventory_management/CRM_Inventory_management_exec.py {} {} {} {} '.format(host,login,password,bucket_name),
            cmd_timeout = 60*1200
        )

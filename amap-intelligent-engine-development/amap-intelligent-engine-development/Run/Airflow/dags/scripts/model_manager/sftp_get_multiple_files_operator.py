                ...
                if self.local_filepath and self.remote_filepath:
                    if isinstance(self.local_filepath, list) and isinstance(self.remote_filepath, str):
                        ...
                        for file_path in self.remote_filepath:
                            ...
                            file_msg = file_path
                            self._transfer(
                                sftp_client, self.local_filepath, remote_file, remote_folder)
                    ...
                    if self.operation.lower() == SFTPOperation.PUT:
                        files_list = self._search_files(
                            os.listdir(self.local_folder))
                        for file in files_list:
                            local_file = os.path.basename(file)
                            file_msg = file
                            self._transfer(
                                sftp_client, self.local_folder, local_file, self.remote_folder)
                    elif self.operation.lower() == SFTPOperation.GET:
                        files_list = self._search_files(
                            sftp_client.listdir(self.remote_folder))
                        for file in files_list:
                            remote_file = os.path.basename(file)
                            file_msg = file
                            self._transfer(
                                sftp_client, self.local_folder, remote_file, self.remote_folder)
                else:
                    raise AirflowException(
                        f"Argument mismatch, please read docs \n {SFTPOperator.__doc__}")
    ...
    def _transfer(self, sftp_client: SFTPClient, local_folder, file, remote_path, only_file=False) -> None:
        ...
        if self.operation.lower() == SFTPOperation.GET:
            ...
            if self.create_intermediate_dirs:
                _make_intermediate_dirs(
                    ...
                    remote_directory=os.path.dirname(
                        remote_path) if only_file else remote_path,
                )
            ...
            if only_file:
                ...
                sftp_client.put(local_full_path,
                                remote_full_path, confirm=self.confirm)

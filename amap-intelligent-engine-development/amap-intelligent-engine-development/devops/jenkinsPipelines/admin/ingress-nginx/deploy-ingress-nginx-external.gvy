pipeline {
    ...
    stages {
        ...
        stage("Configure ingress-nginx external") {
            steps {
                dir("${workspace}/charts/ingress-nginx") {
                    script {
                        sh """
                            ...
                            python3 --version
                            python3 ../../devops/scripts/update_params.py ../../appconfig/${env.ENVIRONMENT}/ingress-nginx.yaml values-external-dm3.yaml values-${env.ENVIRONMENT}.yaml
        ...
        stage("Deploy ingress-nginx external") {
            steps {
                dir("${workspace}/charts/ingress-nginx") {
                    script {
                        sh """
                            ...
                            helm upgrade --install  -n ingress-nginx-ext ingress-nginx-ext . --values=values-${env.ENVIRONMENT}.yaml  --create-namespace \${command_param}
